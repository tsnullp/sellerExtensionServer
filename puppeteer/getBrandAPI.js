...
const getSupersports = async ({ ObjItem, url }) => {
  try {
    ...
    descriptionHtml = descriptionHtml
      ...
      .replace(
        "●予約商品の為、着日・時間帯指定はお受けできません。着日指定の記載があった場合も対応できませんのであらかじめご了承ください。",
        ""
      )
      .replace("●1商品おひとり様1点限りのご購入とさせていただきます。", "")
      .replace(
        "※複数ご注文いただいた場合には、ご希望の商品をメールにてお伺いしその他の商品はキャンセルとさせていただきます。",
        ""
      )
      .replace(
        "なお、以下の場合はご注文は全てキャンセルとさせていただきますのでご注意ください。",
        ""
      )
      .replace(
        "・当店からのご連絡に対し、ご希望商品のご返信をいただけない場合(当店からのメール受信が確認できなかったという場合も含む）",
        ""
      )
      .replace("・注文者および配送先の氏名、または住所が同一の場合", "")
      .replace(
        "●複数回に分けてご注文いただいた場合には初回ご注文のみ受付、その他のご注文は全てキャンセル扱いとさせていただきます。",
        ""
      )
      .replace(
        "当店の検品基準に沿い検品作業を行ったうえで発送いたします。出荷前検品の段階で発送不可との判断に至った場合はご注文をキャンセルさせていただきます。",
        ""
      )
      .replace(
        "●本商品は店舗受け取り指定には対応しておりません。配送先指定のみの対応となります。",
        ""
      )
      .replace(
        "※予約販売時、想定を上回る多数のお客様からのアクセスにより、一時的にカード決済ができない等、障害が起こる場合がございます。",
        ""
      )
      .replace(
        "その場合は、繰り返し実施いただくか、数分待って再度同様のご注文手続きをお願いいたします。",
        ""
      )
      .replace(
        "※こちらの商品につきましては、以下の内容にご同意いただいたお客様に限り、ご注文を承ります。",
        ""
      )
      .replace(
        "●本商品は他の商品との同時注文ができません。他の商品をご注文の際は、本商品と他の商品で2回に分けてご注文をお願いいたします。同時にご注文をいただいた場合、予約商品以外の他商品はキャンセルさせていただきます。あらかじめご了承ください。",
        ""
      );
