...
const getMainKeyword = async (keyword, only=false) => {
  try {
    ...
    const category = await CategoryPredict({userID:"5f0d5ff36fc75ec20d54c40b", productName: title})
    ...
    if(only){
      ...
      result = [...getPermutations(titleArr, 2).map(item => item.join(" ")), regExp_test(myCategory)]
    }
    ...
    const promiseArray = result.map(item => {
      return new Promise(async (resolve, reject) => {
        try {
          const category = await CategoryPredict({userID:"5f0d5ff36fc75ec20d54c40b", productName: item})
    ...
    for(const items of DimensionArray(mainKeyword, 5)){
      ...
      for(const item of items){
        ...
        if(keywordObj){
          mainKeywordArray.push({
            ...
            monthlyMobileQcCnt: Number(keywordObj.monthlyMobileQcCnt.toString().replace("< ", "")),
          })
        }
      }
    ...
    let mainKeyword = ""
      mainKeywordArray = mainKeywordArray.sort((a, b) => 
    ...
    if(mainKeywordArray.length > 0){
      ...
      console.log("조회수:", mainKeywordArray[0].monthlyPcQcCnt + mainKeywordArray[0].monthlyMobileQcCnt)
    }
...
const getCoupnagRelatedKeyword = async ({keyword}) => {
  ...
  try {
    ...
    await page.goto(`https://www.coupang.com/np/search?component=&q=${encodeURI(keyword)}&channel=relate`, { waituntil: "networkidle0"})
...
const getCoupnagAutoKeyword = async ({keyword}) => {
  try {
    ...
    const content = await axios({
      url: `https://www.coupang.com/np/search/autoComplete?callback=jQuery&keyword=${encodeURI(keyword)}`,
      method: "GET",
      headers: {
        // 'Content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
        // 'Accept': '*/*',
...
const searchKeywordTitle = async ({ page, keyword, index = 1 }) => {
  ...
  try {
    ...
    const content = await axios.get(`https://search.shopping.naver.com/api/search/all?sort=rel&pagingIndex=${index}&pagingSize=40&viewType=list&productSet=total&deliveryFee=&deliveryTypeValue=&frm=NVSHOVS&query=${qs.escape(
        keyword
