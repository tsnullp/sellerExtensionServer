  ...
  try {
    ...
    if (!mainImage) {
      const imagesResponse = await Cafe24UploadImages({
        ...
        images: product.mainImages && product.mainImages.length > 0  ? [product.mainImages[0]] : [options[0].image]
      })
      ...
      imagesResponse &&
        ...
        imagesResponse.data.images.forEach((item, index) => {
          if (index === 0) {
            mainImage = item.path.replace(`http://${cafe24.mallID}.cafe24.com`, "").replace(`https://${cafe24.mallID}.cafe24.com`, "")
    ...
    if (product.cafe24_product_no) {
      ...
      cafe24Product = {
        ...
        request: {
          ...
          && (options
          .filter(item => item.active && !item.disabled)[0].korKey === null
          || (
            options
          .filter(item => item.active && !item.disabled)[0].korKey &&
            options
          .filter(item => item.active && !item.disabled)[0].korKey.length === 0))
          ?
            prop.map(item => {
              return {
          ...
          ?
            ...
            .map(item => {
              return {
                ...
                value: item.values
                .filter(valueItem => {
                  const temp = item.values.filter(vItem => vItem.korValueName.trim() === valueItem.korValueName.trim())
          ...
          :[
            {
              ...
              value: options
                ...
                .filter(valueItem => {
                  ...
                  const temp = options.filter(vItem => vItem.korValue.toString().trim() === valueItem.korValue.toString().trim())
                  if(temp.length > 1) {
                    return false
                  }
                  return true
    ...
    options.map((item, i) => {
      ...
      if(isSingle && cafe24ProductsVariants.data && Array.isArray(cafe24ProductsVariants.data.variants) &&
